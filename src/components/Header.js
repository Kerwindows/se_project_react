    ...
    <>
      <header className="header">
        ...
        <div className={`header__menu ${isMenuOpen && "header_menu-open"}`}>
          <p className={`header__email ${!props.userEmail && "hide"}`} >{props.userEmail}</p>
          {props.loggedIn ? <Logout handleLogout={props.handleLogout} setUserEmail={props.setUserEmail} /> : <Link to={location.pathname === "/signin" ? 'signup' : 'signin'} className="header__button">
